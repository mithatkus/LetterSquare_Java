...
public class LetterSquare {
    ...
    private boolean isValid(String letter, int wordNum, int charNum) { 
        ...
        if (charNum == 0 && wordNum == 0){
            return true;
        }


        if (charNum == 0){
            if (letter.equals(lastLetter(this.words[wordNum-1]))){
                return true;
            }
            else{
                return false;
        ...
        if (this.onSameSide(letter, lastLetter(this.words[wordNum]))){
            return false;
        }

        String myWord = this.words[wordNum] + letter;

        if (this.alreadyUsed(myWord)){
            return false;
        }

        if (dictionary.hasString(myWord) == true){
            return true;
        }
    ...
    private boolean solveRB(int wordNum, int charNum, int maxWords) {
        if (dictionary.hasFullWord(this.words[wordNum]) && this.words[wordNum].length() >= 3 && this.allLettersUsed() == true){
        ...
        for (int i = 0; i < this.letters.length; i ++){
            if (this.isValid(this.letters[i], wordNum, charNum) == true){
                addLetter(this.letters[i],wordNum);
                if (this.solveRB(wordNum, charNum + 1, maxWords)){
                    return true;
                }
                else{
                    if (this.words[wordNum].length() >= 3 && dictionary.hasFullWord(this.words[wordNum])){
